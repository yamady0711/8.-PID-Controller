...
//  pid.Init(0.1, 0.0, 1.5); //PD-Controller
  ...
  double speed_limit = 30.0; //initial limit for updating Kp
  int iteration = 0;
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          if( (abs(cte)<2.0)&&(abs(angle)<5.0) ){  // acceleration only when car is under control.
            if(speed < speed_limit * 0.5){
              thr += 0.05 * abs(steer_value);
            }
            else{
              if(speed < speed_limit){thr += 0.01* abs(steer_value);}
              else{thr -= 0.01* abs(steer_value);}
            }
          }
          else{
            if(thr>0.1){thr -= 0.05* abs(steer_value);} // quick deceleration for recovery 
          }
          ...
          cte2 += cte * cte;
          counter += 1;
          if(status==3){ // speed_limit goes up after parameter adjustment completion
